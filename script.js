  ...
  function mediaMarkup(p, cls) {
    if (p.imgs && p.imgs.length > 1) {
      var frames = p.imgs.map(function (src, i) {
        return '<img class="carousel__frame float-img' + (i === 0 ? ' is-on' : '') + '" src="' + res(src) + '" alt="' + p.name + '" loading="lazy" draggable="false" />';
      }).join('');
      return '<div class="' + cls + ' carousel" data-carousel>' + frames + '</div>';
    }
    return '<img class="' + cls + ' float-img" src="' + res(p.img) + '" alt="' + p.name + '" loading="lazy" draggable="false" />';
  }
    ...
    window.openInquiry = function (idx) {
      ...
      if (p.imgs && p.imgs.length > 1) {
        ...
        car.innerHTML = p.imgs.map(function (src, i) {
          return '<img class="carousel__frame float-img' + (i === 0 ? ' is-on' : '') + '" src="' + res(src) + '" alt="' + p.name + '" loading="lazy">';
        }).join('');
